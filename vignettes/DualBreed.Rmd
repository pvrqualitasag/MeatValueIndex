...
## Overview of the population situation
...
Table are presenting economic value in trait unit.
```{r, eval=TRUE, echo=FALSE, results="asis"}
...
## Computing Relative Economic Factors
...
# compute sum of economic values within each breed
...
tbl_rel_fact <- cbind(tbl_ev_result_ev_per_gen_sd[,1],tbl_rel_fact)
knitr::kable(tbl_rel_fact, booktabs = TRUE)
```
...
## Importance of calves versus adults for each population
```{r, eval=TRUE, echo=FALSE, results="asis"}
...
## Economic value weighted by categories
Some population are more used by adults than by calves, or vice versa. Therefore the economie value should be weighted by category. The proportion are calculated bellow:
...
### # Matrix with same dimension as 'tbl_rel_factors'
(tbl_proportion4eachtrait <- rbind(tbl_proportion,tbl_proportion,tbl_proportion))
(tbl_proportion4eachtrait <- tbl_proportion4eachtrait[, 2:ncol(tbl_proportion4eachtrait)])
(tbl_proportion4eachtrait <- cbind(tbl_rel_factors[,1], tbl_proportion4eachtrait))
...
### Determine Relative Factors Based on Given Restrictions

```{r}
library(dplyr)
# tbl_rel_fact %>% filter(Traits == "ccc") %>% select(OB, BV, SI, SF, MO)
# tbl_rel_fact %>% filter(Traits == "cca") %>% select(OB, BV, SI, SF, MO)
tbl_beta <- tbl_rel_fact %>% 
  filter(Traits == "ccc") %>% 
  select(OB, BV, SI, SF, MO) / 
  tbl_rel_fact %>% 
  filter(Traits == "cca") %>% 
  select(OB, BV, SI, SF, MO)
tbl_alphaa <- tbl_rel_fact %>% 
  filter(Traits == "cfa") %>% 
  select(OB, BV, SI, SF, MO) / 
  tbl_rel_fact %>% 
  filter(Traits == "cca") %>% 
  select(OB, BV, SI, SF, MO)
tbl_alphac <- tbl_rel_fact %>% 
  filter(Traits == "cfc") %>% 
  select(OB, BV, SI, SF, MO) / 
  tbl_rel_fact %>% 
  filter(Traits == "ccc") %>% 
  select(OB, BV, SI, SF, MO)
tbl_scale_factor <- rbind(tbl_beta, tbl_alphaa, tbl_alphac)
rownames(tbl_scale_factor) <- c("beta", "alphaa", "alphac")
tbl_scale_factor
```
...
```{r}
cca_new <- 1/(2 + 2*tbl_scale_factor["beta",] + tbl_scale_factor["alphaa",] + tbl_scale_factor["alphac",]*tbl_scale_factor["beta",])
cwa_new <- cca_new
ccc_new <- tbl_scale_factor["beta",] * cca_new
cwc_new <- ccc_new
cfa_new <- cca_new * tbl_scale_factor["alphaa",]
cfc_new <- ccc_new * tbl_scale_factor["alphac",]

tbl_fact_new <- rbind(cca_new, ccc_new, cfa_new, cfc_new, cwa_new, cwc_new)
# rownames(tbl_fact_new) <- c("cca", "ccc", "cfa", "cfc", "cwa", "cwc")
tbl_fact_new <- cbind(c("cca", "ccc", "cfa", "cfc", "cwa", "cwc"), tbl_fact_new)
colnames(tbl_fact_new) <- colnames(tbl_rel_factors)
rownames(tbl_fact_new) <- NULL
```
...
Manual conversion and experiments are shown below.

```{r}
vec_breed <- c("OB", "BV", "SI", "SF", "MO")
# vec_basis <- c("unit", "gen_sd")
vec_basis <- c("gen_sd")
vec_trait <- c("cca", "ccc", "cfa", "cfc", "cwa", "cwc")
n_nr_breed <- length(vec_breed)
n_nr_basis <- length(vec_basis)
...
A good initial test is to write the same tibble as with the manual conversion.

```{r}
MeatValueIndex::write_ev_to_file(ptbl_economic_value = tbl_ev_result_ev_per_gen_sd,
ps_out_path = "economic_value_raw.csv",
pbfirst_col_trait_name = TRUE)
```

As a next set, we are using the relative economic factors
...
The relative factors are weighted with animal categories to get weighted relative factors
...
The third set consist of the creative political values not weighted for animal classes
...
The last set is the above factors weighted with the proportions of the animal classes
...
MeatValueIndex::write_ev_to_file(ptbl_economic_value = tbl_weighted_fact_new,
                                 ps_out_path = "weighted_political_unweighted.csv")
