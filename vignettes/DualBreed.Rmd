...
Some population are more used by adults than by calves, or vice versa. Therefore the economie value should be weighted by category (calves or adults).

```{r}
## convert the tibble with economic values on the basis of one genotypic standard deviation to a #matrix
(mat_nb_a_c <- as.matrix(tbl_number_calves_adults[,2:ncol(tbl_number_calves_adults)]))

## compute sum of category numbers within each breed
(vec_nb <- apply(abs(mat_nb_a_c), 2, sum))

(mat_nb_a_c <- rbind(mat_nb_a_c, vec_nb))
